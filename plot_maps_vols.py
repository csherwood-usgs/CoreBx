...
ax[0,0].text(tx, ty,'a) {}'.format(dates[0]),c='black')
ax[1,0].text(tx, ty,'b) {}'.format(dates[1]),c='black')
ax[2,0].text(tx, ty,'c) {}'.format(dates[2]),c='black')
ax[3,0].text(tx, ty,'d) {}'.format(dates[3]),c='black')
...
##############################################################################
# Elevation plots
#cols = ['dimgray','tab:red','tab:orange','tab:blue']
cols=['#fdbe85','#fd8d3c','#e6550d','#a63603']

title_str = '{0:s} Dune Crest Elevation and Barrier Volume'.format(r['name'].capitalize())
plt.figure(figsize=(12,7))
ax1=plt.subplot(211)
plt.plot(dhi[0,:],color=cols[0],linewidth=3,label=dates[0])
plt.plot(dhi[1,:],color=cols[1],linewidth=2,label=dates[1])
plt.plot(dhi[2,:],color=cols[2],linewidth=2,label=dates[2])
plt.plot(dhi[3,:],color=cols[3],linewidth=2,label=dates[3])

plt.legend(loc='upper right')
ax1.xaxis.set_ticklabels([])
ax1.set_ylim(0,7.5)
ax1.set_xlim(0,r['xlen'])
plt.ylabel('Dune Crest Elevation (m)')
plt.text(0.012*r['xlen'],.9*7.5,"a) Crest Elevations")
plt.title(title_str)

# Volume plot
ax2=plt.subplot(212)
plt.plot([0, nalong*dxdy],[m[0],m[0]],'--',color=cols[0],linewidth=1)
plt.plot([0, nalong*dxdy],[m[1],m[1]],'--',color=cols[1],linewidth=1)
plt.plot([0, nalong*dxdy],[m[2],m[2]],'--',color=cols[2],linewidth=1)
plt.plot([0, nalong*dxdy],[m[3],m[3]],'--',color=cols[3],linewidth=1)
lb = []
for i in range(0,4):
    lb.append("{0} {1:4.0f} m$^3$".format(dates[i],m[i]))
plt.plot(v[0,:],label=lb[0],color=cols[0],linewidth=3)
plt.plot(v[1,:],label=lb[1],color=cols[1],linewidth=2)
plt.plot(v[2,:],label=lb[2],color=cols[2],linewidth=2)
plt.plot(v[3,:],label=lb[3],color=cols[3],linewidth=2)
ax2.set_ylim(0,950)
ax2.set_xlim(0,r['xlen'])
